...
object Game:
  ...
  val emptyState: GameState =
    GameState(GameStateScenario.InbuiltScenario(Expansion.Gloomhaven, 1), Nil, 0, Nil, Nil, Nil, Map.empty, "")
...
final case class Piece(ref: PieceType, x: Int, y: Int)

final case class Cell(rooms: List[MapTileRef], passable: Boolean)
...
final case class RoomData(ref: MapTileRef, origin: (Int, Int), turns: Int)
